...
%{
  ...
  #define cool_yylloc curr_lineno  /* use the curr_lineno from the lexer for the location of tokens */
    ...
    */
    ...
    %type <expression> chained_assignment
    ...
    */
    ...
    dummy_feature_list:	%empty	/* empty */
                        {  $$ = nil_Features(); }
    ...
    chained_assignment:   OBJECTID ':' TYPEID ASSIGN expression IN expression
                          {$$ = let($1,$3,$5, $7);}
                          |OBJECTID ':' TYPEID IN expression
                          {$$ = let($1,$3,no_expr(), $5);}
                          | OBJECTID ':' TYPEID ',' chained_assignment
                          {$$ = let($1,$3,no_expr(),$5);}
                          | OBJECTID ':' TYPEID ASSIGN expression ',' chained_assignment
                          {$$ = let($1,$3,$5,$7);}
                          | error ',' chained_assignment
                          {$$ = $3;}
                          ;
