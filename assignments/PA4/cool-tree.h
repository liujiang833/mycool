...
struct Info;
typedef Info* InfoP;
extern std::unordered_map<Symbol, ClassNodeP> class_table;
extern std::unordered_map<Symbol, SymbolTableP> class_level_tables;
...
std::vector<ClassNodeP> get_path(ClassNodeP);
...
public:
   ...
   virtual int check_and_annotate(SymbolTableP symbol_table, char* filename) = 0;
...
public:
   ...
   Class_ copy_Class_();
   void dump(ostream& stream, int n);
   int check_and_annotate(SymbolTableP symbol_table, char* filename);
