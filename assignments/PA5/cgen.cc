...
void CgenClassTable::build_class_layout(CgenNodeP root) {
  ...
  if (parentnd != nullptr) {
    ...
    curr_layout.self_start = curr_layout.linear_layout.size();
  }
...
void CgenClassTable::build_class_method(CgenNodeP root) {
  ...
  int offset = 0;
  for (auto &method_info : class_methods[root->name].methods) {
    method_offsets[root->name][std::get<1>(method_info)] = offset;
    offset += 4;
  }
  auto children = root->get_children();
  while (children != nullptr) {
...
Symbol ObjectTable::get_classname() {
  if (classname != nullptr) return classname;
  return parent->get_classname();
}

Symbol ObjectTable::get_filename() {
...
void ClassLayout::dump_layout() {
  for (int i = 0; i < linear_layout.size(); i++) {
    ...
    printf("%d: %s %s\n", i, std::get<0>(attr)->get_string(),
           std::get<1>(attr)->get_string());
  }
  printf("------\n");
}
...
static void emit_load(char *dest_reg, int offset, char *source_reg,
                      ostream &s) {
  s << LW << dest_reg << " " << offset << "(" << source_reg << ")" << endl;
}
// static void emit_load(char *dest_reg, int offset, char *source_reg, ostream
// &s) {
...
static void emit_store(char *source_reg, int offset, char *dest_reg,
                       ostream &s) {
  s << SW << source_reg << " " << offset << "(" << dest_reg << ")" << endl;
}
// static void emit_store(char *source_reg, int offset, char *dest_reg, ostream
// &s) {
...
int get_class_tag(Symbol classname) {
  for (int i = 0; i < class_order.size(); i++) {
    if (class_order[i] == classname) return i;
  }
  return -1;
}
...
void CgenClassTable::code_class_init(Symbol classname) {
  ...
  if (classname != Object) {
    std::string parent_init =
        std::string(parent_name->get_string()) + std::string(CLASSINIT_SUFFIX);
  ...
  int offset = 12;
  int pos = 0;
  // attribute that has initialization body
  std::vector<std::pair<attr_class *, int>> attr_and_offset;
  for (auto &attr_info : curr_layout.linear_layout) {
    if (pos >= curr_layout.self_start) {
      attr_class *attr = std::get<2>(attr_info);
      // if attr init body is not empty
      if (typeid(*attr->init) != typeid(no_expr_class)) {
        emit_move(ACC, SELF, str);
        attr->init->object_table = new ObjectTable(curr_node->object_table);
        attr->init->object_table->fp_offset -=12;
        attr->init->code(str);
      } else {
...
void CgenClassTable::code_all_class_methods() { code_class_methods(root()); }

void CgenClassTable::code_class_methods(CgenNodeP root) {
  Features features = root->features;
  std::unordered_set<Symbol> basic_classes{Object, IO, Str, Int, Bool};
  if (basic_classes.find(root->name) == basic_classes.end()) {
    root->code(str);
  }
  auto children = root->get_children();
  while (children != nullptr) {
...
int get_str_const_index(Symbol classname) {
  for (int i = stringtable.first(); stringtable.more(i);
       i = stringtable.next(i)) {
...
void static_dispatch_class::code(ostream &s) {
  ...
  for (int i = actual->first(); actual->more(i); i = actual->next(i)) {
    ...
    one_expr->object_table = object_table;
    one_expr->code(s);
    emit_store(ACC, 0, SP, s);
    emit_addiu(SP, SP, -4, s);
  }
  expr->object_table = new ObjectTable(object_table);
  // we are moving away from fp
  expr->object_table->fp_offset -= actual->len() * 4;
  expr->code(s);
  emit_bne(ACC, ZERO, next_label, s);
  // load file name to $a0
  Symbol filnename = object_table->get_filename();
  emit_load_string(ACC, stringtable.add_string(filnename->get_string()), s);
  // load line number to $t1
  emit_load_imm(T1, line_number, s);
  // call dispatch_abort
  emit_jal("_dispatch_abort", s);
  emit_label_def(next_label++, s);
  std::string method = std::string(type_name->get_string()) + "." +
...
void dispatch_class::code(ostream &s) {
  ...
  Symbol call_type = expr->type;
  if (call_type == SELF_TYPE) call_type = object_table->get_classname();
  emit_load(T1, method_offsets[call_type][name], T1, s);
  emit_jalr(T1, s);
}

void cond_class::code(ostream &s) {
  int local_cond_label = ++cond_label;
  ...
  s << BEQ << ACC << "\t" << ZERO << "\t"
    << "cond_else" << local_cond_label << "\n";
  then_exp->object_table = object_table;
  then_exp->code(s);
  // jump to escape point
  s << BRANCH << "cond_end" << local_cond_label << "\n";
  // false branch
  s << "cond_else" << local_cond_label << ":\n";
  else_exp->object_table = object_table;
  else_exp->code(s);
  // escape point
  s << "cond_end" << local_cond_label << ":\n";
}

void loop_class::code(ostream &s) {
  int local_loop_label = ++loop_label;
  // entry point of loop
  s << "loop_begin" << local_loop_label << ":\n";

  // compute predicate
  pred->object_table = object_table;
  pred->code(s);
  emit_load(ACC, 12, ACC, s);
  // emit_beq(ACC, ZERO, loop_label, s);
  s << BEQ << ACC << " " << ZERO << " "
    << "loop_end" << local_loop_label << "\n";
  // loop body
  body->object_table = object_table;
  body->code(s);
  // back to entry point
  // emit_branch(loop_label - 1, s);
  s << BRANCH << "loop_begin" << local_loop_label << "\n";
  // escape point
  s << "loop_end" << local_loop_label << ":\n";
}
...
void eq_class::code(ostream &s) {
  ...
  emit_move(T2, ACC, s);
  emit_move(T1, "$s1", s);
  // check if two are the same object
  emit_sub(T3, T1, T2, s);
  emit_beq(T3, ZERO, next_label, s);
  // branch value
  emit_load_imm(ACC, 0, s);
...
void new__class::code(ostream &s) {
  ...
  if (type != SELF_TYPE) {
    std::string address =
        std::string(type->get_string()) + std::string(PROTOBJ_SUFFIX);
    emit_load_address(ACC, (char *)address.c_str(), s);
    emit_jal("Object.copy", s);
    address = std::string(type->get_string()) + std::string(CLASSINIT_SUFFIX);
...
void typcase_class::code(ostream &s) {
  int local_branch_label = ++branch_label;
  expr->object_table = object_table;
  expr->code(s);
  s << BEQ << ACC << "\t" << ZERO << "\t"
    << "abort_void" << local_branch_label << endl;
  ;
  // push the object onto stack
  emit_push(ACC, s);
  int count = 0;
  for (int i = cases->first(); cases->more(i); i = cases->next(i)) {
    // generate label for current branch
    s << "branch" << local_branch_label << "_" << count << ":\n";
    branch_class *one_case = (branch_class *)cases->nth(i);
    // load class tag of the object
    emit_load(T1, 0, ACC, s);
    // load branch class tag
    emit_load_imm(T2, get_class_tag(one_case->type_decl), s);
    // if not equal jump to next branch
    s << BNE << T1 << "\t" << T2 << "\t"
      << "branch" << local_branch_label << "_" << count + 1 << endl;
    one_case->expr->object_table = new ObjectTable(object_table);
    one_case->expr->object_table->add_object(one_case->name,
                                             object_table->fp_offset, FP);
    // prepare next offset
    one_case->expr->object_table->fp_offset -= 4;
    one_case->expr->code(s);
    // jump to end of case
    s << BRANCH << "case_end" << local_branch_label << endl;
    count++;
  }
  auto classname =
      stringtable.add_string(object_table->get_classname()->get_string());
  // abort on no match
  s << "branch" << local_branch_label << "_" << count << ":\n";
  emit_move(ACC, SELF, s);
  emit_jal("_case_abort", s);
  // abort on void
  s << "abort_void" << local_branch_label << ":\n";
  emit_load_imm(T1, get_line_number(), s);
  auto filename =
      stringtable.add_string(object_table->get_filename()->get_string());
  emit_load_string(ACC, filename, s);
  emit_jal("_case_abort2", s);
  // end of branch
  s << "case_end" << local_branch_label << ":\n";
  emit_addiu(SP, SP, 4, s);
}
...
void class__class::code(ostream &s) {
  ...
  object_table->filename = filename;
  ClassLayout &layout = class_layouts[name];
  int offset = 12;
  // add class attributes to symbol table
  for (auto &attr : layout.linear_layout) {
    object_table->add_object(std::get<0>(attr), offset, SELF);
    offset += 4;
  }
  object_table->add_object(self, 0, SELF);

  // generate all methods definition
  // for simplicity, inherited methods are
  // also generated in child class.
  for (auto &item : class_methods[name].methods) {
    method_class *method = std::get<2>(item);
...
void method_class::code(ostream &s, Symbol classname) {
  ...
  emit_load(RA, 4, SP, s);
  // restore stack - formal and three slots
  emit_addiu(SP, SP, formals->len() * 4 + 12, s);
  emit_return(s);
}

void dump_method_offsets() {
  for (auto &c_mo : method_offsets) {
    cout << c_mo.first << ":" << endl;
    for (auto &m_o : c_mo.second) {
      cout << "\t" << m_o.first << "\t" << m_o.second << endl;
    }
  }
}
